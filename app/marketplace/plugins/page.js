...
const PluginsPage = () => {
  ...
  const { data: pluginTagsData, isFetching: pluginTagsLoading } =
    useGetToolUniqueFiltersQuery({ type: "plugin" });
  ...
  useEffect(() => {
    if (pluginsData?.tools) {
      setPluginDataState((prev) => [...prev, ...pluginsData.tools]);
      setHasMore(currentPage < Math.ceil(pluginsData.total_tools / perPage));
      setIsLoading(false);
    }
  }, [pluginsData]);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore && !pluginsLoading) {
    ...
    <>
      <div className="mytools  xl:mt-[65px] 2xl:max-w-[1207px] xl:max-w-[905px] lg:max-w-[905px]  w-full mx-auto">
        <div className="toolsHeading mb-[47px]">
          <h3 className="text-white text-[32px] font-bold helvetica-font mb-[14px]">{`Plugins`}</h3>
          ...
          <div className="flex flex-row gap-2 mt-[20px] max-msm:mt-0 mb-4 max-msm:ml-3 items-center h-[35px] max-w-[500px]">
            <Input
              type="text"
              placeholder="Search for plugins like “Image generators”"
              classNames={{
                label: "text-white",
                input: [
                  "bg-[transparent]",
          ...
          <div className="flex relative ">
            ...
            >
              {pluginTagsLoading ? (
                ...
                <>
                  {pluginTagsData?.tags?.[0]?.uniqueTags?.map((item, index) => (
                    <Button
                      onClick={() => {
          ...
          <div className="py-[17px]">
            <div className="w-full">
              ...
              <div className="">
                ...
                pluginDataState?.length < perPage ? (
                ...
                ) : pluginDataState?.length > 0 ? (
                  <div className="grid grid-cols-12 gap-5 p-4">
                    {pluginDataState?.map((item, index) => (
                      <div
                      ...
                      >
                        ...
                        <MarketplaceCard
                          ...
                          refetchToolListOnHome={refetchPluginData}
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="w-full flex items-center justify-center mt-10">
                    <p className="text-white">No Tools found.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        >
          {hasMore && pluginDataState?.length >= perPage && (
