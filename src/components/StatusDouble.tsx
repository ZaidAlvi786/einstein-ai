...
const getClassNameByStatus = (status?: Status, tab?: string) => {
  ...
  if (tab === 'application') {
    ...
    if (variant === 'schedueled') return 'bg-Brand-50 border-Brand-200 text-Brand-700';
    if (variant === 'neutral' && status?.text === 'Qualified')
...
export function Status({ statusValues, tab }: Props) {
  return (
    <div className="flex flex-wrap gap-2">
      {statusValues &&
        statusValues.length > 0 &&
        statusValues.map((statusValue, index) => {
          const status = statuses.find((item) => item.value.includes(statusValue)); 
